(() => {
  ...
  // ประกาศ state ให้เป็น Global เพื่อให้ฟังก์ชัน UI ดึงไปใช้ต่อได้
  ...
  // --- 2. CORE BOOTLOADER (ลำดับ: DB -> VAULT -> SYNC) ---
  async function bootSystem() {
    console.log('🚀 FAKDU System Booting...');
    try {
      // ขั้นที่ 1: โหลด Offline DB
      await window.FakduDB.ready();
      let localDb = await window.FakduDB.load();
      if (!localDb) {
        localDb = window.FakduVault.ensureDbShape({});
        await window.FakduDB.save(localDb);
      }
      window.state.db = localDb;

      // ขั้นที่ 2: ตรวจสอบ License & Security (ล็อก Machine ID + Signed Token)
      // ระบบจะใช้วิธีเช็กลายเซ็น ห้ามแฮกผ่านตัวแปร boolean เด็ดขาด
      window.state.hwid = await window.FakduVault.getInstallId();
      const vaultStatus = await window.FakduVault.getStatus(window.state.db);
      window.state.isPro = vaultStatus.active; 
      window.state.shopId = vaultStatus.shopId;

      // ขั้นที่ 3: เปิดระบบ Firebase Sync (Online / Anti-Theft / เชื่อมเครื่องลูก)
      if (window.FakduFirebaseSync) {
        window.state.syncApi = window.FakduFirebaseSync.resolveApi();
        if (window.state.syncApi && window.state.shopId) {
          console.log('📡 Sync Module Connected:', window.state.shopId);
          
          // ดักฟังความเคลื่อนไหวจากเครื่องลูก หรือเวลา Sync ข้อมูล
          window.state.syncApi.listenOperations(window.state.shopId, Date.now(), (op) => {
            if (op && op.sourceHwid !== window.state.hwid) {
               console.log('🔄 Data Synced from Cloud:', op);
               // สั่งให้ UI โหลดข้อมูลใหม่ (ถ้ามีฟังก์ชัน refresh ใน UI)
      ...
      console.log(`✅ FAKDU Core Ready! [PRO STATUS: ${window.state.isPro ? 'ACTIVE' : 'FREE'}]`);
      
      // ปลุก UI ให้ทำงาน (เรียกฟังก์ชันโหลดหน้าจอเดิมของคุณ)
      if(typeof window.renderAll === 'function') window.renderAll();
      
    } catch (err) {
      console.error('❌ Boot Error:', err);
      alert('เกิดข้อผิดพลาดในการโหลดระบบฐานข้อมูล กรุณารีเฟรชหน้าเว็บ');
    }
  }

  // สั่งรัน Core ทันทีที่โหลดไฟล์เสร็จ
  document.addEventListener('DOMContentLoaded', bootSystem);

  // --- 3. EXPORT FOR UI ---
  // ฟังก์ชันเซฟกลาง เพื่อให้ฟังก์ชันปุ่มกดต่างๆ เรียกใช้ได้ง่ายๆ
  window.saveDatabase = async () => {
    if (!window.state.db) return;
    await window.FakduDB.save(window.state.db);
    
    // Auto-Sync ขึ้น Cloud ถ้าเป็นรุ่น PRO
    if (window.state.syncApi && window.state.isPro) {
       window.state.syncApi.writeSnapshot(window.state.shopId, window.state.db);
    }
  };
